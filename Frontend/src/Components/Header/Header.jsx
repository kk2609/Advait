...
const disclaimerText = "Mutual Fund investments are subject to market risks, read all scheme related documents carefully before investing.";
...
const Header = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (index < disclaimerText.length) {
        setDisplayedText(disclaimerText.slice(0, index + 1));
        index++;
      } else {
  ...
  return (
    <>
      {/* Disclaimer Top Bar */}
      {disclaimerOpen && (
        <div className="w-full bg-dark-color">
          <div className="mx-auto flex max-w-auto items-center justify-between gap-2 sm:gap-4 px-4 py-2.5 sm:px-6 lg:px-10 xl:px-16">
            {/* Left side: Icon and Label */}
            <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
              <FaShieldAlt className="shrink-0 text-xl text-primary-color sm:text-2xl" />
              <span className="hidden sm:inline font-body-font text-xs sm:text-sm font-semibold text-white whitespace-nowrap">
                {/* Mutual Fund Disclaimer: */}
              </span>
            </div>

            {/* Middle: Disclaimer Text with Typewriter Animation */}
            <div className="flex-1 overflow-hidden">
              <p className="font-body-font text-xs sm:text-sm text-white whitespace-nowrap sm:whitespace-normal min-h-[1.2em]">
                {displayedText}
                {displayedText.length < disclaimerText.length && (
            ...
            <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0">
              {/* <Link
                to="#"
                className="hidden sm:inline font-body-font text-xs sm:text-sm text-white font-semibold underline hover:opacity-80 transition-opacity duration-200 whitespace-nowrap"
              >
                Read Disclaimer
              </Link> */}
              ...
              <button
                ...
                className="flex items-center justify-center text-white hover:opacity-80 transition-opacity duration-200 p-1"
            ...
            <div className="sm:hidden flex items-center gap-2">
              <button
                onClick={() => setDisclaimerOpen(false)}
                className="flex items-center justify-center text-white hover:opacity-80 transition-opacity duration-200"
                aria-label="Close disclaimer"
              >
                <HiX className="text-lg" />
              </button>
